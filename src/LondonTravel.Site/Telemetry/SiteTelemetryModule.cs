...
namespace MartinCostello.LondonTravel.Site.Telemetry
{
    ...
    internal sealed class SiteTelemetryModule : ITelemetryModule, IDisposable
    {
        ...
        /// <summary>
        /// The diagnostic source listener in use for HTTP calls.
        /// </summary>
        private HttpDiagnosticSourceListener _httpListener;
        ...
        public void Initialize(TelemetryConfiguration configuration)
        {
            if (!_initialized)
            {
                lock (_syncRoot)
                {
                    if (!_initialized)
                    {
                        _httpListener = new HttpDiagnosticSourceListener(configuration);
                        _initialized = true;
                    }
                }
            }
